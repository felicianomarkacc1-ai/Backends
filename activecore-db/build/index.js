...
// ===== HELPER FUNCTIONS =====
async function enhanceAIWeekPlanWithDetails(parsedWeekPlan, dishes) {
...
async function safeOpenAICompletionsCreate(params, timeoutMs = 12000) {
    if (!openai) {
...
async function ensureUserPreferenceExists(userId) {
    try {
        const [rows] = await db_config_1.pool.query('SELECT id FROM user_meal_preferences WHERE user_id = ? LIMIT 1', [userId]);
        if (rows && rows.length > 0)
            return rows[0].id;
        const [insertResult] = await db_config_1.pool.query('INSERT INTO user_meal_preferences (user_id, preferences, created_at) VALUES (?, ?, NOW())', [userId, JSON.stringify({})]);
...
app.get('/api/health', async (req, res) => {
    try {
        await db_config_1.pool.query('SELECT 1');
        res.json({
            status: 'ok',
...
app.get('/api/system/status', async (req, res) => {
    try {
        ...
        try {
            await axios_1.default.get(PAYMONGO_BASE_URL + '/v1/sources', {
                timeout: 1500,
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        const [users] = await db_config_1.pool.query('SELECT * FROM users WHERE email = ?', [email]);
...
app.post('/api/auth/change-password', async (req, res) => {
    try {
        ...
        const [users] = await db_config_1.pool.query('SELECT * FROM users WHERE id = ?', [userId]);
        if (!Array.isArray(users) || users.length === 0) {
        ...
        await db_config_1.pool.query('UPDATE users SET password = ? WHERE id = ?', [hashedPassword, userId]);
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        const [existingUsers] = await db_config_1.pool.query('SELECT id FROM users WHERE email = ?', [email]);
        ...
        const [result] = await db_config_1.pool.query(`INSERT INTO users (
...
app.get('/api/members', async (req, res) => {
    try {
        ...
        const [members] = await db_config_1.pool.query(`SELECT 
        u.id, 
        u.email, 
...
app.put('/api/members/:id', authenticateToken, async (req, res) => {
    try {
        ...
        await db_config_1.pool.query(`UPDATE users SET ${updateFields.join(', ')} WHERE id = ?`, updateValues);
...
app.delete('/api/members/:id', async (req, res) => {
    try {
        ...
        const [result] = await db_config_1.pool.query('DELETE FROM users WHERE id = ? AND role = "member"', [id]);
...
// ===== PAYMENT ROUTES =====
app.get('/api/member/subscription', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const [member] = await db_config_1.pool.query(`SELECT 
...
app.post('/api/member/payment/gcash', authenticateToken, async (req, res) => {
    try {
        ...
        const [result] = await db_config_1.pool.query(`INSERT INTO payments (
        ...
        await db_config_1.pool.query(`UPDATE users 
       SET status = 'active',
           payment_status = 'paid',
           subscription_start = ?,
           subscription_end = ?,
...
app.post('/api/admin/payments/record-cash', authenticateToken, async (req, res) => {
    try {
...
app.get('/api/admin/payments/all', authenticateToken, async (req, res) => {
    try {
        const [payments] = await db_config_1.pool.query(`
      SELECT 
        p.id,
        p.user_id,
        p.amount,
...
app.get('/api/admin/payments/summary', authenticateToken, async (req, res) => {
    try {
        ...
        const [revenueRows] = await db_config_1.pool.query(`
      SELECT SUM(amount) as totalRevenue
      FROM payments
      WHERE payment_status = 'paid'
    `);
        // Count of pending payments
        const [pendingRows] = await db_config_1.pool.query(`
      SELECT COUNT(*) as pendingPayments
      FROM payments
      WHERE payment_status = 'pending'
    `);
        // Count of paid payments
        const [paidRows] = await db_config_1.pool.query(`
...
app.post('/api/meal-planner/generate', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const [dbDishes] = await db_config_1.pool.query('SELECT * FROM filipino_dishes ORDER BY name ASC');
...
`;
        ...
        try {
            const [prefRows] = await db_config_1.pool.query('SELECT id FROM user_meal_preferences WHERE user_id = ?', [userId]);
        ...
        try {
            ...
            await db_config_1.pool.query(`INSERT INTO meal_plans (${insertCols}) VALUES (${qMarks})`, insertVals);
...
app.post(['/api/meal-planner/regenerate', '/meal-planner/regenerate'], authenticateToken, async (req, res) => {
    try {
        ...
        let dishes = [];
        if (category) {
            const [rows] = await db_config_1.pool.query('SELECT * FROM filipino_dishes WHERE category = ?', [category]);
            dishes = rows || [];
        }
        if (!Array.isArray(dishes) || dishes.length === 0) {
            const [rows] = await db_config_1.pool.query('SELECT * FROM filipino_dishes ORDER BY name');
            dishes = rows || [];
        }
...
async function dbColumnExists(table, column) {
    try {
        const dbName = process.env.DB_NAME || 'activecore';
        const [rows] = await db_config_1.pool.query(`SELECT COUNT(*) as cnt 
...
app.post('/api/meal-planner/save', authenticateToken, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId)
        ...
        if (!isNaN(requestedPrefId) && requestedPrefId > 0) {
            try {
                const [rows] = await db_config_1.pool.query('SELECT id, user_id FROM user_meal_preferences WHERE id = ? LIMIT 1', [requestedPrefId]);
        ...
        const conn = await db_config_1.pool.getConnection();
        try {
            ...
            if (timestampCol)
                insertColsArr.push(timestampCol);
            ...
            const [insertResult] = await conn.query(`INSERT INTO meal_plans (${insertCols}) VALUES (${qMarks})`, insertVals);
            await conn.commit();
            conn.release();
...
app.get('/api/meal-planner/plans', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const [rows] = await db_config_1.pool.query(`SELECT ${cols.join(', ')} FROM meal_plans WHERE user_id = ? ORDER BY ${orderBy} DESC`, [userId]);
...
app.get('/api/meal-planner/plans/:id', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        if (hasUpdatedAt)
            cols.push('updated_at');
        const [rows] = await db_config_1.pool.query(`SELECT ${cols.join(', ')} FROM meal_plans WHERE id = ?`, [planId]);
        if (!rows || rows.length === 0) {
...
app.delete('/api/meal-planner/plans/:id', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const planId = Number(req.params.id);
        // verify existence & owner (select minimal columns)
        const [rows] = await db_config_1.pool.query('SELECT id, user_id FROM meal_plans WHERE id = ?', [planId]);
        if (!rows || rows.length === 0) {
            return res.status(404).json({ success: false, message: 'Plan not found' });
        }
        ...
        await db_config_1.pool.query('DELETE FROM meal_plans WHERE id = ?', [planId]);
...
app.post('/api/attendance/checkin', authenticateToken, async (req, res) => {
    try {
        ...
        const [existing] = await db_config_1.pool.query(`SELECT id FROM attendance WHERE user_id = ? AND DATE(check_in_time) = ?`, [userId, todayStr]);
        if (existing.length > 0) {
            return res.status(400).json({ success: false, message: "Already checked in today." });
        }
        // Insert attendance record
        await db_config_1.pool.query(`INSERT INTO attendance (user_id, check_in_time, location, status) VALUES (?, NOW(), ?, 'present')`, [userId, location || 'Main Gym']);
        res.json({
            success: true,
...
app.get('/api/attendance/history', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const [rows] = await db_config_1.pool.query(`SELECT id, check_in_time, location, status FROM attendance WHERE user_id = ? ORDER BY check_in_time DESC`, [userId]);
        // Format for frontend
...
app.get('/api/admin/attendance/today', authenticateToken, async (req, res) => {
    try {
        ...
        const [rows] = await db_config_1.pool.query(`SELECT a.id, a.user_id, a.check_in_time, a.location, u.first_name, u.last_name, u.email
       FROM attendance a
...
app.get('/api/rewards/available', authenticateToken, async (req, res) => {
    try {
        ...
        const [claimedRows] = await db_config_1.pool.query(`SELECT reward_id, claimed_at FROM rewards_claimed WHERE user_id = ?`, [userId]);
        const claimedMap = new Map();
        claimedRows.forEach(r => claimedMap.set(r.reward_id, r.claimed_at));
        // Fetch attendance count
        const [attendanceRows] = await db_config_1.pool.query(`SELECT COUNT(*) as total FROM attendance WHERE user_id = ?`, [userId]);
...
app.post('/api/rewards/claim', authenticateToken, async (req, res) => {
    try {
        ...
        const [claimedRows] = await db_config_1.pool.query(`SELECT id FROM rewards_claimed WHERE user_id = ? AND reward_id = ?`, [userId, rewardId]);
        if (claimedRows.length > 0) {
            return res.status(400).json({ success: false, message: "Reward already claimed." });
        }
        // Insert claim
        await db_config_1.pool.query(`INSERT INTO rewards_claimed (user_id, reward_id, claimed_at) VALUES (?, ?, NOW())`, [userId, rewardId]);
...
app.get('/api/user/profile', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        const [users] = await db_config_1.pool.query('SELECT id, email, first_name, last_name, role FROM users WHERE id = ?', [userId]);
...
async function startServer() {
    ...
    try {
        try {
            dbConnected = await (0, db_config_1.initializeDatabase)();
...
app.post('/api/admin/qr-token/generate', authenticateToken, async (req, res) => {
    try {
...
async function notifyInactiveMembers(thresholdDays = 3) {
    try {
        ...
        const [rows] = await db_config_1.pool.query(`
      ...
      `, [thresholdDays]);
        ...
        for (const u of rows) {
            ...
            const [alreadySent] = await db_config_1.pool.query(`SELECT id FROM notification_logs WHERE user_id = ? AND type = 'absent_reminder' AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY) LIMIT 1`, [u.id, thresholdDays]);
      ...
      `;
            ...
            if (sent) {
                await db_config_1.pool.query(`INSERT INTO notification_logs (user_id, type, created_at) VALUES (?, 'absent_reminder', NOW())`, [u.id]);
                notifiedCount++;
            }
            else {
...
app.post('/api/admin/attendance/notify-inactive', authenticateToken, async (req, res) => {
    try {
...
app.post('/api/admin/attendance/test-email', authenticateToken, async (req, res) => {
    try {
        const { to } = req.body;
        if (!to) {
...
app.post('/api/payments/paymongo/create-source', authenticateToken, async (req, res) => {
    try {
        const userId = req.user.id;
        ...
        const response = await axios_1.default.post(`${PAYMONGO_BASE_URL}/sources`, payload, {
            headers: {
        ...
        await db_config_1.pool.query(`INSERT INTO payments (user_id, amount, payment_method, membership_type, payment_status, transaction_id, created_at)
...
app.post('/api/payments/paymongo/webhook', express_1.default.raw({ type: 'application/json' }), async (req, res) => {
    try {
        ...
        const [rows] = await db_config_1.pool.query(`SELECT id, user_id, amount, membership_type FROM payments WHERE transaction_id IN (?, ?) LIMIT 1`, [sourceId, paymentId]);
        if (!rows || rows.length === 0) {
        ...
        if (eventType === 'payment.failed')
            newStatus = 'failed';
        await db_config_1.pool.query(`UPDATE payments SET payment_status = ?, transaction_id = ?, payment_date = NOW() WHERE id = ?`, [newStatus, paymentId || sourceId, record.id]);
        if (newStatus === 'completed') {
            ...
            if (/quarter/i.test(plan))
                months = 3;
            await db_config_1.pool.query(`UPDATE users SET next_payment = DATE_ADD(CURDATE(), INTERVAL ? MONTH) WHERE id = ?`, [months, record.user_id]);
            await db_config_1.pool.query(`INSERT INTO payments_history (user_id, payment_id, amount, payment_method, status, created_at)
...
app.get('/api/payments/paymongo/verify', authenticateToken, async (req, res) => {
    try {
        ...
        const [rows] = await db_config_1.pool.query(`SELECT * FROM payments WHERE transaction_id = ? LIMIT 1`, [sourceId]);
        if (!rows || rows.length === 0) {
